...
def getPhoto(place_id):
    response = safe_requests.get(f"https://maps.googleapis.com/maps/api/place/details/json?place_id={place_id}&fields=photo&key={MAPS_KEY}")
...
def getReviews(place_id):
    response = safe_requests.get(f"https://maps.googleapis.com/maps/api/place/details/json?place_id={place_id}&fields=reviews&key={MAPS_KEY}")
    result = response.json()['result']
...
for college in colleges:
    for type in types:
        ...
        response = safe_requests.get(f"https://maps.googleapis.com/maps/api/place/nearbysearch/json?radius=2500&type={type}&key={MAPS_KEY}&location={cords}")
